<template>
  <section class="max-w-xl space-y-5">
    ...
    <form
      class="space-y-4 rounded-xl bg-background p-6 shadow"
    ...
    >
      ...
      /><input
        ...
        class="w-full rounded border p-3"
      /><button class="rounded bg-primary px-5 py-3 text-primary-foreground">
        Update password
      </button>
      <p>{{ message }}</p>
    </form>
...
const submit = async () => {
  if (!valid(newPassword.value)) {
    message.value =
      'Password must be 12+ characters with upper/lowercase, a number, and a symbol.';
    return;
  }
  try {
    ...
    message.value = 'Unable to update password.';
  }
};
</script>
